...
#ifdef PER_FACE_LIGHTING
        vec4 tintColor = gl_FrontFacing ? vertexPerFaceColorFront : vertexPerFaceColorBack;
#else
        vec4 tintColor = vertexColor;
#endif
        vec4 final_color = textureLod(Sampler0, atlasUV, 0.0) * tintColor;
